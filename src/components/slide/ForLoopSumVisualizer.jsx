...
const ForLoopSumVisualizer = () => {
    ...
    const [instruction, setInstruction] = useState("Press 'Run Step' to start the sum demonstration");
    ...
    const code = [
        ...
        { line: 3, sumInitialization: `int sum = 0;` },
        {
            line: 4,
            text: `    for(`,
        ...
        { line: 5, text: `        sum = sum + i;` },
        { line: 6, text: `        printf("Current sum: %d\\n", sum);` },
        { line: 7, text: `    }` },
        { line: 8, text: `    printf("Final sum: %d\\n", sum);` },
        { line: 9, text: `    return 0;` },
        { line: 10, text: `}` },
    ];
    ...
    const steps = [
        {
            line: 0,
            description: "Initialize sum variable",
            execute: () => {
                if (!hasInitialized) {
                    setVariables({ i: loopStart, sum: 0 });
                    setInstruction(`Initializing sum to 0`);
        ...
        },
        {
            ...
            execute: () => {
                if (variables.i > loopEnd) {
                    ...
                    // Add final sum to output when loop is completed
                    setOutput(prev => [...prev, `Final sum: ${variables.sum}`]);
        ...
        {
            line: 1,
            description: "Add current value to sum",
            execute: () => {
                setVariables(prev => ({
                    ...prev,
                    sum: prev.sum + prev.i
                }));
                setInstruction(`Adding ${variables.i} to sum.`);
            },
        },
        {
            line: 2,
            description: "Print current sum",
            execute: () => {
                if (!loopCompleted) {
                    setOutput((prev) => {
                        const currentSumMessage = `Current sum: ${variables.sum}`;
                        return prev.some(item => item === currentSumMessage)
                            ? prev
                            : [...prev, currentSumMessage];
                    });
                    setInstruction(`Printing current sum: ${variables.sum}`);
                }
            },
        },
        {
            ...
            execute: () => {
                ...
                setVariables((prev) => ({ ...prev, i: newValue }));
    ...
    const resetLoop = () => {
        ...
        setVariables({ i: loopStart, sum: 0 });
        setOutput([]);
        setInstruction("Press 'Run Step' to start the sum demonstration");
    ...
    return (
        <div className="w-full max-w-5xl mx-auto bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl shadow-2xl text-white p-6">
            <div className="grid md:grid-cols-2 gap-6">
                ...
                >
                    <h3 className="text-2xl font-semibold mb-4 text-blue-300">Code Visualization</h3>
                    <div className="bg-slate-800 p-6 rounded-xl border-2 border-blue-500/30">
                        {code.map((line) => (
                            <div
                                key={line.line}
                                className={`flex items-center font-mono text-xl mb-1 transition-all duration-300 ${
                                    currentStep === -1 && line.line === 2
                                        ? "text-green-300 font-bold scale-105"
                                        : currentStep % steps.length === 2 && line.line === 5
                                            ? "bg-blue-900/50 text-blue-200"
                                            : currentStep % steps.length === 3 && line.line === 6
                                                ? "bg-green-900/50 text-green-200 animate-pulse"
                                                : loopCompleted && line.line === 8
                                                    ? "bg-blue-900/50 text-blue-200 animate-pulse font-bold"
                                                    : "text-gray-300"
                                }`}
                                >
                                <span className="mr-3 text-gray-500 text-lg">{line.line}.</span>
                                <span>{line.text}</span>
                                {line.sumInitialization && (
                                    <span
                                        className={`${
                                            currentStep === 0
                ...
                <motion.div
                    ...
                    transition={{ duration: 0.5, delay: 0.2 }}
                >
                    <h3 className="text-2xl font-semibold mb-4 text-green-300">Output Stream</h3>
                    <div className="bg-slate-800 p-6 rounded-xl border-2 border-green-500/30 min-h-[250px]">
                        <AnimatePresence>
                            {output.map((value, index) => (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, x: 0 }}
                                    exit={{ opacity: 0, x: 20 }}
                                    className="flex items-center text-green-300 text-xl mb-1"
                                >
                                    <span className="mr-3 text-gray-500 text-lg">{index + 1}.</span>
                                    <span>{value}</span>
                                </motion.div>
                            ))}
                        </AnimatePresence>
            ...
            <div className="mt-6 grid md:grid-cols-2 gap-6">
                ...
                >
                    <h3 className="text-2xl font-semibold mb-4 text-purple-300">Current Variables</h3>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            >
                                <div className="flex items-center">
                                    <CodeIcon />
                                    <span className="text-xl font-medium">Variable sum</span>
                                </div>
                                <div className="bg-slate-700 rounded-lg p-2 text-center">
                                    <span className="text-xl font-mono text-blue-300">{variables.sum}</span>
                                </div>
                            </motion.div>
                            <motion.div
                                key="i-variable"
                            ...
                            >
                                ...
                                <div className="bg-slate-700 rounded-lg p-2 text-center">
                                    <span className="text-xl font-mono text-green-300">{variables.i}</span>
                                </div>
                            </motion.div>
                        </div>
                    </div>
                ...
                <motion.div
                    ...
                    transition={{ duration: 0.5, delay: 0.4 }}
                >
                    <h3 className="text-2xl font-semibold mb-4 text-yellow-300">Current Instruction</h3>
            ...
            >
                ...
                <div className="flex items-center gap-4">
                    <button
                        onClick={runStep}
                        disabled={loopCompleted && currentStep !== steps.length - 1}
...
export default ForLoopSumVisualizer;